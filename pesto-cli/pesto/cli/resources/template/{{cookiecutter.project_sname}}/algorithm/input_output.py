...
@dataclass
class Input:
    image: np.array = definition(
        Definition.Image, required=True, description="Input image"
    )
    dict_parameter: dict = definition(
        Definition.Metadata, description="A dict parameter", default_factory=dict
    )
    object_parameter: object = definition(
        Definition.Metadata,
    ...
    number_parameter: float = field(
        description="A (floating point) number parameter", default=None
    )
...
class Output:
    ...
    image_list: List[np.array] = definition(
        Definition.Images, description="The output images"
    )
    geojson: object = user_definition("geojson")
    dict_parameter: dict = definition(
        Definition.Metadata, description="A dict parameter", default=None
    )
